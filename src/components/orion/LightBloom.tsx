...
export default function LightBloom(props: Props) {
  ...
  useEffect(() => {
    ...
    // Em celulares e em usuários com redução de movimento, usamos o fallback CSS.
    // Isso evita perda intermitente de contexto WebGL em GPUs móveis sem mudar a aparência do rodapé.
    const lightweight = window.matchMedia("(max-width: 767px), (prefers-reduced-motion: reduce)").matches;
    if (lightweight) {
    ...
    const schedule = () => {
      if (!raf && inView && pageVisible && !contextLost) raf = requestAnimationFrame(render);
    };

    const render = (now: number) => {
      raf = 0;
      if (!inView || !pageVisible || contextLost) return;
      ...
      const cw = sizeRef.current.w || canvas.clientWidth || 1;
      const ch = sizeRef.current.h || canvas.clientHeight || 1;
  ...
  return (
    ...
    >
      <div
        className={`orion-light-bloom-fallback orion-light-bloom-${direction}`}
        style={{
          position: "absolute",
          inset: 0,
          backgroundColor: background,
          ["--orion-bloom-bg" as string]: background,
          ["--orion-bloom-base" as string]: baseColor,
          ["--orion-bloom-accent" as string]: accentColor,
        }}
      />
      <canvas
        ref={canvasRef}
