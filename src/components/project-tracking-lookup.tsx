...
export default function ProjectTrackingLookup() {
  ...
  return (
    <section className="relative mt-12 overflow-hidden rounded-[2rem] border border-cyan-300/30 bg-slate-950/72 p-6 shadow-[0_0_60px_rgba(34,211,238,0.08),0_0_100px_rgba(139,92,246,0.06)] backdrop-blur-2xl md:p-8">
      <div aria-hidden className="pointer-events-none absolute inset-x-12 top-0 h-px bg-gradient-to-r from-transparent via-cyan-300/90 to-transparent" />
      <div aria-hidden className="pointer-events-none absolute -right-28 -top-28 h-64 w-64 rounded-full bg-violet-500/10 blur-3xl" />
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[1fr_360px]">
          <div className="space-y-5">
            <div className="rounded-2xl border border-cyan-300/30 bg-gradient-to-br from-cyan-400/[0.08] to-violet-500/[0.04] p-5 shadow-[0_0_32px_rgba(34,211,238,0.06)]">
              ...
              <div className="mt-3 h-3 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-cyan-300 via-blue-400 to-violet-400 shadow-[0_0_16px_rgba(34,211,238,0.55)] transition-all duration-700" style={{ width: `${tracking.progress}%` }} /></div>
            ...
            {tracking.timeline?.length ? (
              <div className="max-h-[430px] space-y-3 overflow-y-auto pr-1">
                {tracking.timeline.map((item) => (
                  <article key={item.id} className="rounded-xl border border-cyan-300/15 bg-white/[0.03] p-4 transition hover:border-cyan-300/35 hover:bg-cyan-300/[0.045] hover:shadow-[0_0_20px_rgba(34,211,238,0.06)]">
